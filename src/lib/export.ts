import { AnalysisResult, ExportOptions, Recommendation } from '@/types'
...
export class ExportManager {
  ...
  private static formatRecommendation(rec: string | Recommendation): string {
    if (typeof rec === 'string') {
      return rec;
    }
    // For Recommendation objects, use the action field as the main text
    return rec.action;
  }
...
`
    ...
    if (result.overallAssessment) {
      ...
      markdown += `**${result.overallAssessment.level.toUpperCase()}** - ${result.overallAssessment.message}\n\n`
      
      if (result.overallAssessment.strengths && result.overallAssessment.strengths.length > 0) {
    ...
    if (options.includeRecommendations) {
      markdown += `## Top Recommendations\n\n${result.summary.recommendations.map((rec, idx) => `${idx + 1}. ${this.formatRecommendation(rec)}`).join('\n')}\n\n`
    }
    ...
    if (options.includeDetails) {
      ...
      result.categories.forEach(category => {
        ...
        if (category.strengths && category.strengths.length > 0) {
          markdown += `**Strengths (${category.strengths.length}):**\n\n`
        ...
        if (category.implementationTasks && category.implementationTasks.length > 0) {
          markdown += `**Implementation Tasks (${category.implementationTasks.length}):**\n\n`
...
<body>
    ...
    <div class="recommendations">
        ...
        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; margin: 20px 0;">
            <h4 style="color: ${
              result.overallAssessment.level === 'excellent' ? '#059669' :
              result.overallAssessment.level === 'good' ? '#0369a1' :
              result.overallAssessment.level === 'moderate' ? '#d97706' :
              '#dc2626'
            }; margin-top: 0;">${result.overallAssessment.level.toUpperCase()}</h4>
            <p>${result.overallAssessment.message}</p>
            ${result.overallAssessment.strengths && result.overallAssessment.strengths.length > 0 ? `
            <div style="margin-top: 15px;">
                <h5 style="color: #059669; margin-bottom: 10px;">🌟 Key Strengths</h5>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 10px;">
                    ${result.overallAssessment.strengths.map(strength => `
    ...
    <div class="recommendations">
        ...
        <ol>
            ${result.summary.recommendations.map(rec => `<li>${this.formatRecommendation(rec)}</li>`).join('')}
        </ol>
    </div>
    ` : ''}
    ...
    <div class="category">
        ...
        <div style="margin: 15px 0;">
            <strong>🌟 Strengths (${category.strengths.length}):</strong>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 8px; margin-top: 10px;">
                ${category.strengths.map(strength => `
                <div style="background: #ecfdf5; padding: 10px; border-radius: 6px; border-left: 4px solid #10b981; font-size: 14px;">
                    ✅ ${strength}
                </div>
                `).join('')}
            </div>
        ...
        <div style="margin: 15px 0;">
            ...
            <ul>
                ${category.recommendations.map(rec => `<li>${this.formatRecommendation(rec)}</li>`).join('')}
            </ul>
        </div>
        ` : ''}
        ...
        <div style="margin: 15px 0;">
            <strong>🎯 Implementation Tasks (${category.implementationTasks.length}):</strong>
            <div style="margin-top: 10px;">
                ${category.implementationTasks.map(task => {
                  const cleanTask = task.replace(/^\[\s*\]\s*/, '') // Remove checkbox prefix
                  return `
                  <div style="background: #eff6ff; padding: 10px; margin: 5px 0; border-radius: 6px; border-left: 4px solid #3b82f6;">
                      <input type="checkbox" style="margin-right: 8px;" disabled> ${cleanTask}
                  </div>
                  `
                }).join('')}
            </div>
